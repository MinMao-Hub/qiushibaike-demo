...
- (void)viewDidLoad
{
    ...
    statusbar.image = [UIImage imageNamed:@"videos_nav_bg"];
...
- (void)setWebView
{
    ...
    NSURL *url = [NSURL URLWithString:@"http://m.video.so.com/?partner=m_home"];
...
{
    //添加导航栏
    UIImageView *myNavBar = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, kScreemWidth, 40)];
    myNavBar.userInteractionEnabled = YES;
    myNavBar.image = [UIImage imageNamed:@"videos_nav_bg"];
    [webView addSubview:myNavBar];
    
    //添加标题
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake((self.view.frame.size.width - 130)/2, 0, 130, 40)];
    ...
    backButton.frame = CGRectMake(2, 5, 30, 30);
