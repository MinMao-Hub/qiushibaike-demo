...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.alpha = 0.f;
        UIImage *shareBgImageg = [UIImage imageNamed:@"forward_background"];
        imageViewBg= [[UIImageView alloc]initWithImage:shareBgImageg];
        [imageViewBg setFrame:CGRectMake(0, 0 , shareBgImageg.size.width/2,shareBgImageg.size.height/2)];
        imageViewBg.center = CGPointMake(self.center.x, -self.center.y);
        imageViewBg.userInteractionEnabled = YES;
        [self addSubview:imageViewBg];
        
        NSArray *array = [NSArray arrayWithObjects:@"sina",@"tencent",@"renren",@"qzone",@"mail",@"copy",nil];
